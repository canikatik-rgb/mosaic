...
// Get welcome node content based on language - REFACTORED
function getWelcomeNodeContent() { // Removed lang parameter
    // Build the HTML using getText function from ui.js
    const html = `
...
function createNode(x, y, content = null, nodeType = 'default', id = null, stripColor = null) {
    ...
    // Pass necessary parameters: id, position, content, and initial strip color
    const effectiveStripColor = stripColor || stripColors[0]; // Use provided color or default
    const node = nodeLoader(nodeId, x, y, content, effectiveStripColor);
    ...
    if (!strip || !contentDiv || !leftPin || !rightPin) {
        console.warn(`Node type '${nodeType}' might be missing standard elements (strip, content, pin). Event listeners might not work correctly.`);
        // Decide how to handle this: continue with warnings or stop?
    }
    ...
    setupNodeEvents(node, strip, contentDiv, leftPin, rightPin);
...
function setupNodeEvents(node, strip, contentDiv, leftPin, rightPin) {
    // Start Node dragging
    ...
    if (strip) {
        strip.addEventListener('dblclick', e => {
            // Ensure click is directly on strip
            if (e.target === strip) {
                e.stopPropagation();
                showColorPalette(strip);
            }
        });
    }
    ...
    if (contentDiv) {
        ...
        // Add click handler to contentDiv as well
    ...
    function handleNodeSelection(e) {
        ...
        // Always allow selecting with shift key, even with single click
        if (shiftKeyDown || e.shiftKey) {
            // If node is already selected, deselect it, otherwise select it
            if (selectedNodes.has(node)) {
                deselectNode(node);
            } else {
                selectNode(node, true); // true = multi-select
            }
        } else {
...
function moveNode(e) {
    ...
    if (selectedNodes.size > 0) {
        selectedNodes.forEach(selectedNode => {
            // Get current position of each selected node
            ...
            selectedNode.style.left = `${currentX + dx / window.canvasScale}px`;
            selectedNode.style.top = `${currentY + dy / window.canvasScale}px`;
        });
    } else {
        // If no multi-selection, just move the dragged node
        const canvasPos = getNodeCanvasPosition(e);
        draggedNode.style.left = `${canvasPos.x - nodeOffsetX / window.canvasScale}px`;
        draggedNode.style.top = `${canvasPos.y - nodeOffsetY / window.canvasScale}px`;
    }
    ...
    // Update connections - using throttled version to improve performance
    if (window.updateNodeConnectionsThrottled) {
        updateNodeConnectionsThrottled();
...
function stopNodeDrag() {
    ...
    let moveAction = null;
    if (!isDuplicating && window.actionHistory && !window.actionHistory.isPerformingAction) {
        moveAction = createMoveActions();
        if (moveAction) {
...
function setStripColor(strip, color) {
    ...
    const node = strip.parentElement;
...
function setupContentEditing(contentDiv, node) {
    ...
    contentDiv.addEventListener('blur', () => {
        ...
        if (contentChanged && window.actionHistory && !window.actionHistory.isPerformingAction) {
            ...
            window.actionHistory.addAction(action);
            originalContent = placeholder; // Update baseline to placeholder
            // Schedule auto-save since content changed (deleted to placeholder)
            if (window.scheduleAutoSave) window.scheduleAutoSave();
        }
        // --- END REFINEMENT ---
    });
...
function duplicateSelectedNodes() {
    ...
    selectedNodes.forEach(node => {
        ...
        if (nodeType === 'checklist' && node.getContentData) {
            // Use the node's method to get structured data if available
            content = node.getContentData();
            console.log('Duplicating checklist data:', content);
        } else {
            // Check if it's placeholder text for other node types
            const placeholder = getPlaceholderText();
            content = content === placeholder ? null : content;
        ...
        // Temporarily disable history for the initial creation
        const wasPerformingAction = window.actionHistory ? window.actionHistory.isPerformingAction : false;
        if (window.actionHistory) window.actionHistory.isPerformingAction = true;

        // Create a duplicate node with all properties
        const newNode = createNode(
            x + offsetX,
            y + offsetY,
            content,
            nodeType,   // Pass node type
            null,       // Let createNode generate a new ID
            stripColor  // Pass strip color
        );

        // Restore history state
        if (window.actionHistory) window.actionHistory.isPerformingAction = wasPerformingAction;

        // Copy visibility mode
