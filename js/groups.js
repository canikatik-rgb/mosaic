...
function selectGroup(group) {
    ...
    if (selectedGroup && selectedGroup.element) {
        selectedGroup.element.classList.remove('selected');
    }
    ...
    // Notify Action Bar (we'll need to dispatch a custom event)
    window.dispatchEvent(new CustomEvent('groupSelected', { detail: { group } }));
}
...
function showGroupEditor(groupId) {
    ...
    panel.innerHTML = `
        ...
        <div style="display: flex; gap: 12px; justify-content: flex-end;">
            <button id="group-delete-btn" style="padding: 10px 18px; border: none; border-radius: 10px; font-size: 14px; font-weight: 500; cursor: pointer; background: rgba(255, 68, 68, 0.1); color: #ff4444; transition: all 0.2s;">Delete</button>
    ...
    panel.querySelector('#group-delete-btn').addEventListener('click', () => {
        if (confirm(`Delete group "${group.name}"? Nodes will not be deleted.`)) {
            deleteGroup(groupId);
            closeEditor();
        }
    });
